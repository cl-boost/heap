...
(defpackage :boost-heap
  ...
  (:export
   ...
   ;; find and remove an element in the heap
   #:heap-find
   #:heap-find-if
...
(defun heap-remove (heap &optional (i 0))
  "Remove the item at the given index from the heap."
  (with-slots (compare elts)
      heap
    (loop
       with root = (aref elts i)

       ;; put the last element into this index
       with last = (setf (aref elts i) (vector-pop elts))
...
(defun heap-pop (heap)
  ...
  (with-slots (compare elts)
    ...
    (cond ((= (length elts) 0) nil)
          ...
          (t (heap-remove heap 0)))))

;;; ----------------------------------------------------

(defun heap-find (x heap &key key (test 'eql))
  "Locate and remove an item on the heap."
  (let ((i (position x (heap-elts heap) :key key :test test)))
    (when i
      (heap-remove heap i))))

;;; ----------------------------------------------------

(defun heap-find-if (pred heap &key key)
  "Local and remove the first item to match a predicate."
  (let ((i (position-if pred (heap-elts heap) :key key)))
    (when i
      (heap-remove heap i))))
